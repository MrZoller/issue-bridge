...
from app.models import ProjectPair, SyncLog, Conflict, SyncedIssue
...
def get_dashboard_stats(db: Session = Depends(get_db)):
    ...
    recent_successes = db.query(SyncLog).filter(
        SyncLog.created_at >= last_24h,
        SyncLog.status == SyncStatus.SUCCESS
    ).count()
    recent_failures = db.query(SyncLog).filter(
        SyncLog.created_at >= last_24h,
    ...
    for pair in pairs:
        synced_issues_count = db.query(SyncedIssue).filter(
            SyncedIssue.project_pair_id == pair.id
        ).count()
        conflicts_count = db.query(Conflict).filter(
            Conflict.project_pair_id == pair.id,
            Conflict.resolved == False
        ).count()

        last_log = db.query(SyncLog).filter(
            SyncLog.project_pair_id == pair.id
